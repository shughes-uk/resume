...
# the plan role only from pull_request workflows (fork PRs never get an OIDC token),
# and the preview role can only write to the PR preview bucket.
...
data "aws_iam_policy_document" "github_assume_preview" {
  statement {
    actions = ["sts:AssumeRoleWithWebIdentity"]
    principals {
      type        = "Federated"
    ...
    # The deploy job runs in the preview environment; the cleanup job runs without an
    # environment so it doesn't create a deployment of its own.
    condition {
      test     = "StringEquals"
      variable = "token.actions.githubusercontent.com:sub"
      values = [
        "repo:${local.github_repo}:environment:preview",
...
resource "aws_iam_role" "github_deploy_preview" {
  name                 = "github-deploy-preview"
  assume_role_policy   = data.aws_iam_policy_document.github_assume_preview.json
  max_session_duration = 3600
}

data "aws_iam_policy_document" "github_deploy_preview" {
  statement {
    actions   = ["s3:ListBucket"]
    resources = [module.preview_s3_bucket.s3_bucket_arn]
  }
  statement {
    actions   = ["s3:PutObject", "s3:DeleteObject"]
    resources = ["${module.preview_s3_bucket.s3_bucket_arn}/*"]
...
resource "aws_iam_role_policy" "github_deploy_preview" {
  name   = "deploy-preview"
  role   = aws_iam_role.github_deploy_preview.id
  policy = data.aws_iam_policy_document.github_deploy_preview.json
}
...
output "github_deploy_preview_role_arn" {
  value     = aws_iam_role.github_deploy_preview.arn
  sensitive = true
}
